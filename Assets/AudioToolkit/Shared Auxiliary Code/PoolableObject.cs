        ...
        internal void _SetAvailable( PoolableObject poolObj, bool b )
        {
            ...
            if ( b )
            {
                if ( poolObj.sendAwakeStartOnDestroyMessage )
                {
                    ...
                    poolObj.gameObject.SendMessage( "OnDestroy", null, SendMessageOptions.DontRequireReceiver );
                }
                poolObj.gameObject.name = "pooled:" + poolObj._myPool._prefabPoolObj.name;
                poolObj.gameObject.active = false;
            }
            else
            {
                poolObj.gameObject.active = true;
                
                if ( poolObj.sendAwakeStartOnDestroyMessage )
                {
                    poolObj.gameObject.SendMessage( "Awake", null, SendMessageOptions.DontRequireReceiver );
                    if ( poolObj.gameObject.active && // Awake could deactivate object
                         poolObj._wasStartCalledByUnity  // for preloaded objects Unity will call Start
                        ) 
                    {
                        poolObj.gameObject.SendMessage( "Start", null, SendMessageOptions.DontRequireReceiver );
                    }
                }
